    ...
    for i in range(base - 1):
        for j in range(logB_ell):
            if ((i + 1) * base ** j - 1 < minibin_capacity):
                if zog == 0:
                    print("#############")
                    print(type(received_enc_query_serialized[i][j]))
                    print(len(received_enc_query_serialized[i][j]))
                    print("#############")
                    zog += 1
                received_enc_query[i][j] = ts.bfv_vector_from(srv_context, received_enc_query_serialized[i][j])
                if zog == 1:
                    print(received_enc_query[i][j].data)
                    print(type(received_enc_query[i][j]))
                    print(received_enc_query[i][j].size())
                    zog += 1
